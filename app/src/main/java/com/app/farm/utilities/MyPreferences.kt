...
class MyPreferences(appContext: Context) {
    ...
    // Saving User Data
    fun putUserData(key: String?, obj: LoginResponseModel?) {
        checkForNullKey(key)
        val gson = Gson()
    ...
    // Get User Data
    fun getUserData(key: String?): LoginResponseModel? {
        val gson = Gson()
        val objStrings = getString(key)
        return gson.fromJson(objStrings, LoginResponseModel::class.java)
    }
